...
export default function GeneratedDocument() {
  ...
  const handleSign = async () => {
    ...
    setReviewError('');
    try {
      const { data } = await signDocument(id, { full_name: signName });
      setDoc(data);
      setSignName('');
    } catch (err) {
  ...
  const needsReview = reviewStatus && !['approved', 'not_required'].includes(reviewStatus);
  ...
  return (
    <div className="p-4 md:p-8 max-w-[95rem] mx-auto space-y-8 bg-slate-50/50 min-h-[calc(100vh-4rem)]">
      ...
      {needsReview && (
        <div className="bg-red-600 text-white px-6 py-3 rounded-2xl text-sm font-bold tracking-wide flex items-center gap-3 shadow-sm uppercase">
          <ShieldAlert size={18} className="shrink-0" />
          Draft — Pending Review ({REVIEW_STATUS_LABELS[reviewStatus] || reviewStatus})
        </div>
      )}
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 md:p-8 space-y-6">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
              <ShieldCheck size={20} className="text-indigo-500" /> Review &amp; Signature
            </h3>
          ...
          {canActAsSupervisor && reviewStatus === 'pending_supervisor' && (
            <div className="pt-4 border-t border-slate-100 space-y-3">
              <p className="text-sm font-semibold text-slate-700">Supervisor Decision</p>
              <textarea
